...
class th_grade_report_grader extends grade_report_grader {
	/**
	 * The final grades.
	 * @var array $grades
	 */
	 ...
	 * @param string $mode whole, aggregatesonly, gradesonly
	 */

	public function __construct($courseid, $gpr, $context, $page = null, $sortitemid = null, $collapsed = null) {
		global $CFG;
		...
		// $this->gtree = new grade_tree($this->courseid, true, false);
	...
	// 	$prefix2 = 'ej' . $i . '_ue';
	// 	$sql = "SELECT DISTINCT {$prefix}u.id,{$prefix2}.timestart,{$prefix2}.timeend,{$prefix2}.timecreated,{$prefix2}.timemodified
	//              FROM {user} {$prefix}u
	//            $capjoin->joins
	 ...
	 */
	// public function load_users($cohortid = null, $time_from = null, $time_to = null, $sorder = null) {
	// 	global $CFG, $DB;
	...
	// 		$sql = "SELECT ue.userid, ue.timestart, ue.timeend, ue.timecreated, ue.timemodified
	//                FROM {user_enrolments} ue
	//                JOIN {enrol} e ON e.id = ue.enrolid
	//               	WHERE ue.userid $usql
	//                     AND ue.status = :uestatus
	//                     AND e.status = :estatus
	//                     AND e.courseid = :courseid
	//                     AND (ue.timestart > :timefrom1 OR (ue.timestart = 0 AND ue.timecreated > :timefrom2))
	//                     AND (ue.timestart < :timeend1 OR (ue.timestart = 0 AND ue.timecreated < :timeend2))
	//                     AND e.enrol ='cohort' and e.customint1 = :cohortid
	//            	GROUP BY ue.userid";

	// 		$params = array_merge($uparams, array('estatus' => ENROL_INSTANCE_ENABLED, 'uestatus' => ENROL_USER_ACTIVE,
	// 			'courseid' => $coursecontext->instanceid,
	// 			'timefrom1' => $time_from, 'timefrom2' => $time_from,
	// 			'timeend1' => $time_to, 'timeend2' => $time_to, 'cohortid' => $cohortid));

	// 		$usersintime = $DB->get_records_sql($sql, $params);
	// 		$results = array();
	// 		foreach ($this->users as $user) {
	// 			if (array_key_exists($user->id, $usersintime)) {
	...
	public function load_users($time_from = null, $time_to = null, $user_status = null, $sorder = null) {
		global $CFG, $DB;
		...
		if ($time_from != null && $time_to != null && $user_status) {
			...
			$sql = "SELECT ue.userid, ue.timestart, ue.timeend, ue.timecreated, ue.timemodified
		              ...
		              JOIN {user} us ON us.id = ue.userid
		             	WHERE ue.userid $usql
		                   ...
		                   AND e.courseid = :courseid
		                   AND (ue.timestart > :timefrom1 OR (ue.timestart = 0 AND ue.timecreated > :timefrom2))
		                   AND (ue.timestart < :timeend1 OR (ue.timestart = 0 AND ue.timecreated < :timeend2)) $wheresql_userstatus
		          	GROUP BY ue.userid";

			$params = array_merge($uparams, array('estatus' => ENROL_INSTANCE_ENABLED, 'uestatus' => ENROL_USER_ACTIVE,
				'courseid' => $coursecontext->instanceid,
				'timefrom1' => $time_from, 'timefrom2' => $time_from,
				'timeend1' => $time_to, 'timeend2' => $time_to));
		...
		// if ($userids_arr != null) {
		// 	$usersintime = [];
		// 	foreach ($userids_arr as $key => $value) {
		// 		$usersintime[$value] = $value;
		// 	}
		// 	$results = array();
		// 	foreach ($this->users as $user) {
		// 		if (in_array($user->id, $usersintime)) {
		// 			$results[$user->id] = $user;
		// 		}
		// 	}
		// 	$this->users = $results;
	...
	public function get_left_rows($displayaverages) {
		...
		foreach ($this->users as $userid => $user) {
			...
			$usercell->fullname = $usercell->text;
			$userrow->fullname = $usercell->text;
			...
			$userrow->userreportcell = $userreportcell->text;
		...
		foreach ($this->gtree->get_levels() as $key => $row) {
			...
			foreach ($row as $columnkey => $element) {
				...
				if ($type == 'filler' or $type == 'fillerfirst' or $type == 'fillerlast') {
					...
					$itemcell->text = $headerlink . $singleview;
		...
		foreach ($this->users as $userid => $user) {
			...
			foreach ($this->gtree->items as $itemid => $unused) {
				...
				if (!empty($grade->finalgrade)) {
					...
					$itemrow->finalgrade = grade_format_gradevalue($grade->finalgrade, $item, true, $gradedisplaytype, null);
					$gradevalforjs = null;
				...
				// }
				$itemcell->gradeval = grade_format_gradevalue($gradeval, $item, true, $gradedisplaytype, null);
				$itemrow->cells[] = $itemcell;
			}
			$rows[] = $itemrow;
		}
	...
	public function get_full_table($displayaverages = false) {
	...
	public function get_grade_table($displayaverages = false) {
		...
		$fulltable = $this->get_full_table($displayaverages);
		$html = '';
	...
	/**
	 * Gets the sql fragment for user
	 *
	 * @return     <type>  The user in or equal.
	 */
	public function get_user_in_or_equal() {
		global $DB;
		if (!empty($this->users)) {
			list($usql, $uparams) = $DB->get_in_or_equal(array_keys($this->users), SQL_PARAMS_NAMED, 'usid0');
		} else {
			list($usql, $uparams) = array('', array());
		}
		return array($usql, $uparams);
	}

	/**
	 * override base method to get only num users filtered by date of enrol
	 *
	 * @param      boolean  $groups  The groups
	 * @param      boolean  $users   The users
	 *
	 * @return     integer  The numusers.
	 */
	public function get_numusers($groups = true, $users = false) {
		...
		list($usql, $uparams) = $this->get_user_in_or_equal();
	...
	protected function get_course_header($element) {
		...
		if ($element['type'] == 'category') {
			...
			if (!in_array($element['object']->id, $this->collapsed['aggregatesonly'])) {
				$this->collapsed['aggregatesonly'][] = $element['object']->id;
			}
		}
	...
	public function get_sort_arrows(array $extrafields = array()) {
		...
		if (!empty($requirednames)) {
			foreach ($requirednames as $name) {
				$arrows['studentname'] .= $this->get_lang_string($name);
