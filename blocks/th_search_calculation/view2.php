...
$pageurl = '/blocks/th_search_calculation/view2.php';
$context = context_system::instance();
$title = get_string('title', 'block_th_search_calculation');
$PAGE->set_url('/blocks/th_search_calculation/view2.php');
...
$editurl = new moodle_url('/blocks/th_search_calculation/view2.php');
...
$table->head = array('STT', 'Tên khóa học', 'Tên rút gọn khóa học', 'Công thức bài kiểm tra khóa học');
$stt = 0;

foreach ($listcourses as $k => $course) {
	$course_id = $course->id;

	$id = $DB->get_field_sql("SELECT id FROM {grade_items} WHERE courseid = '$course_id' AND idnumber = 'kt'");
	...
	if (!empty($id)) {
		...
		if (!empty($calculation)) {
			...
			$cell = new html_table_cell("<span style = 'color: #fff'>Không tìm thấy công thức bài kiểm tra</span>");
			$row->cells[] = $cell;
...
$baseurl = new moodle_url('/blocks/th_search_calculation/view2.php');
