...
class main implements renderable, templatable {
	 ...
	 *
	 * @var string String matching the grouping constants defined in th_activatecourses/lib.php
	 */
	private $grouping;
	 ...
	 *
	 * @var string String matching the sort constants defined in th_activatecourses/lib.php
	 */
	private $sort;
	 ...
	 *
	 * @var string String matching the view/display constants defined in th_activatecourses/lib.php
	 */
	private $view;
	 ...
	 *
	 * @var string String matching the paging constants defined in th_activatecourses/lib.php
	 */
	private $paging;
	...
	/**
	 * Store the configuration values for the th_activatecourses block.
	 *
	 * @var array Array of available layouts matching view/display constants defined in th_activatecourses/lib.php
	 */
	private $layouts;
	...
	public function __construct($grouping, $sort, $view, $paging, $customfieldvalue = null) {
		...
		if ($sort) {
			...
			$this->sort = BLOCK_th_activatecourses_SORTING_SHORTNAME;
		} else {
			$this->sort = BLOCK_th_activatecourses_SORTING_TITLE;
		}
		// In case sorting remembered is shortname and display extended course names not checked,
		// we should revert sorting to title.
		if (!$CFG->courselistshortnames && $sort == BLOCK_th_activatecourses_SORTING_SHORTNAME) {
			$this->sort = BLOCK_th_activatecourses_SORTING_TITLE;
		}

		// Check and remember the given view.
		$this->view = $view ? $view : BLOCK_th_activatecourses_VIEW_CARD;

		// Check and remember the given page size, `null` indicates no page size set
		// while a `0` indicates a paging size of `All`.

		if (!is_null($paging) && $paging == BLOCK_th_activatecourses_PAGING_ALL) {
			$this->paging = BLOCK_th_activatecourses_PAGING_ALL;
		} else {
			$this->paging = $paging ? $paging : 3;
		}

		// Check and remember if the course categories should be shown or not.
		if (!$config->displaycategories) {
			$this->displaycategories = BLOCK_th_activatecourses_DISPLAY_CATEGORIES_OFF;
		} else {
			$this->displaycategories = BLOCK_th_activatecourses_DISPLAY_CATEGORIES_ON;
		}
	...
	private function get_fallback_grouping($config) {
		if ($config->displaygroupingall == true) {
			return BLOCK_th_activatecourses_GROUPING_ALL;
		}
		if ($config->displaygroupingallincludinghidden == true) {
			return BLOCK_th_activatecourses_GROUPING_ALLINCLUDINGHIDDEN;
		}
		if ($config->displaygroupinginprogress == true) {
			return BLOCK_th_activatecourses_GROUPING_INPROGRESS;
		}
		if ($config->displaygroupingfuture == true) {
			return BLOCK_th_activatecourses_GROUPING_FUTURE;
		}
		if ($config->displaygroupingpast == true) {
			return BLOCK_th_activatecourses_GROUPING_PAST;
		}
		if ($config->displaygroupingfavourites == true) {
			return BLOCK_th_activatecourses_GROUPING_FAVOURITES;
		}
		if ($config->displaygroupinghidden == true) {
			return BLOCK_th_activatecourses_GROUPING_HIDDEN;
		}
		if ($config->displaygroupingcustomfield == true) {
			return BLOCK_th_activatecourses_GROUPING_CUSTOMFIELD;
		}
	...
	public function set_available_layouts() {
		...
		if ($config = get_config('block_th_activatecourses', 'layouts')) {
			$this->layouts = explode(',', $config);
		} else {
			$this->layouts = array(BLOCK_th_activatecourses_VIEW_CARD);
		}
	}
	...
	public function format_layout_for_export($layoutname) {
		...
		$layout->name = get_string($layoutname, 'block_th_activatecourses');
		$layout->active = $this->view == $layoutname ? true : false;
		$layout->arialabel = get_string('aria:' . $layoutname, 'block_th_activatecourses');
	...
	public function get_customfield_values_for_export() {
		...
		$customfieldactive = ($this->grouping === BLOCK_th_activatecourses_GROUPING_CUSTOMFIELD);
		$ret = [];
	...
	public function export_for_template(renderer_base $output) {
		...
		$nocoursesurl = $output->image_url('courses', 'block_th_activatecourses')->out();

		$customfieldvalues = $this->get_customfield_values_for_export();
		$selectedcustomfield = '';
		if ($this->grouping == BLOCK_th_activatecourses_GROUPING_CUSTOMFIELD) {
			...
			if (!$selectedcustomfield) {
				$this->grouping = $this->get_fallback_grouping(get_config('block_th_activatecourses'));
				if ($this->grouping == BLOCK_th_activatecourses_GROUPING_CUSTOMFIELD) {
		...
		$sort = '';
		if ($this->sort == BLOCK_th_activatecourses_SORTING_SHORTNAME) {
			$sort = 'shortname';
		} else {
			$sort = $this->sort == BLOCK_th_activatecourses_SORTING_TITLE ? 'fullname' : 'ul.timeaccess desc';
		}
		...
		if (count($courseidarr) == 0) {
			global $PAGE, $COURSE;
			if ($config->displayemptyblockonlyforadmin) {
				$coursecontext = context_course::instance($COURSE->id);
				if (!has_capability('moodle/site:config', $coursecontext)) {
					$PAGE->requires->js_call_amd('block_th_activatecourses/main', 'hideemptyblock');
				}
			} else {
				$PAGE->requires->js_call_amd('block_th_activatecourses/main', 'hideemptyblock');
			}
		...
		$defaultvariables = [
			'totalcoursecount' => count($courseidarr),
