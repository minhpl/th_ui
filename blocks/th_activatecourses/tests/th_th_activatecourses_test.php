 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_th_activatecourses_testcase extends advanced_testcase {
	...
	public function test_get_block_config_for_external() {
		...
		set_config('displaygroupingcustomfield', 1, 'block_th_activatecourses');
		set_config('customfiltergrouping', $field->get('shortname'), 'block_th_activatecourses');
		...
		foreach ($blocks as $region => $regionblocks) {
			...
			foreach ($regioninstances as $ri) {
				// Look for th_activatecourses block only.
				if ($ri->instance->blockname == 'th_activatecourses') {
		...
		$this->assertEquals(get_config('block_th_activatecourses', 'version'), $configs->plugin->version);
		// Test custom fields.
